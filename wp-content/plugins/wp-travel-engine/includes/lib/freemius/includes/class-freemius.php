...
class Freemius extends Freemius_Abstract {
	...
	private function get_caller_main_file_and_type( $module_id ) {
		...
		for ( $i = 1, $bt = debug_backtrace(), $len = count( $bt ); $i < $len; $i++ ) {
			...
			if ( ! empty( $bt[ $i ]['function'] ) && in_array(
				$bt[ $i ]['function'],
				array(
					'do_action',
					'apply_filter',
	...
	function _add_deactivation_feedback_dialog_box() {
		...
		if ( $show_deactivation_feedback_form ) {
			...
			if ( $is_long_term_user ) {
				$user_type = 'long-term';
			} elseif ( ! $this->is_registered() && ! $this->is_anonymous() ) {
	...
	function is_clone( $only_if_manual_resolution_is_not_hidden = false ) {
		...
		$site_url = self::get_unfiltered_site_url( $blog_id, true, true );
	...
	function dynamic_init( array $plugin_info ) {
		...
		if ( ! $this->is_registered() ) {
			if ( $this->is_anonymous() ) {
				...
				$this->_is_on              = true;
			} elseif ( false === $this->has_api_connectivity() ) {
					return;
			} elseif ( $this->_anonymous_mode ) {
			...
			if ( $this->is_addon() ) {
				if ( ! $this->is_parent_plugin_installed() ) {
					...
					if ( $this->is_premium() ) {
						...
						$this->_parent->_admin_notices->remove_sticky(
							array(
	...
	private function get_api_error_message( $result ) {
		$error_message = sprintf(
			$this->get_text_inline(
				'There was an unexpected API error while processing your request. Please try again in a few minutes and if it still doesn\'t work, contact the %s\'s author with the following:',
	...
	function _plugin_code_type_changed() {
		...
		if ( $this->is_premium() ) {
			...
			$this->_admin_notices->remove_sticky(
				array(
					'trial_started',
					'plan_upgraded',
					'plan_changed',
	...
	function is_anonymous() {
		if ( ! isset( $this->_is_anonymous ) ) {
			...
			} elseif ( ! isset( $this->_storage->is_anonymous ) ) {
					// Not skipped.
					$this->_is_anonymous = false;
									...
									sprintf(
										...
										sprintf(
											$this->get_text_x_inline(
												...
												'%s - plugin name. As complete "PluginX" activation now',
												'activate-x-now'
											),
											$this->get_plugin_name()
										)
			...
			<script type="text/javascript">// <![CDATA[
				jQuery(document).ready(function ($) {
					if ('undefined' !== typeof(jQuery().pointer)) {
						...
						<?php
						echo $this->apply_filters(
							'optin_pointer_execute',
							"
	...
	function _deactivate_plugin_hook() {
		...
		} elseif ( false === $this->has_api_connectivity() && ! $this->is_premium() ) {
	...
	private function update_license_required_permissions_if_anonymous() {
		...
		FS_Permission_Manager::instance( $this )->update_permissions_tracking_flag(
			array(
				'essentials' => true,
				'events'     => true,
				'diagnostic' => false,
	...
	private function maybe_set_slug_and_network_menu_exists_flag() {
		if ( ! empty( $this->_dynamically_added_top_level_page_hook_name ) ) {
			$this->_menu->set_slug_and_network_menu_exists_flag(
				$this->_menu->has_menu() ?
				$this->_menu->get_slug() :
				$this->_slug
			);
	...
	function _get_addon_info( $addon_id, $is_installed ) {
		...
		if ( isset( $all_plans[ $slug ] ) ) {
			...
			foreach ( $plans as $plan ) {
				if ( $site->plan_id == self::_decrypt( $plan->id ) ) {
					$addon_info['plan_name']  = self::_decrypt( $plan->name );
					$addon_info['plan_title'] = self::_decrypt( $plan->title );
					break;
				}
			}
		}
	...
	private function activate_license(
		...
		if ( is_object( $user ) ) {
			...
			} elseif ( $is_network_activation_or_migration ) {
					/**
	...
	function _get_admin_page_url( $page = '', $params = array(), $network = null ) {
		...
		if ( ! $this->has_settings_menu() ) {
			...
			} elseif ( $this->is_activation_mode() ) {
					/**
					 * @author Vova Feldman
					 * @since  1.2.1.6
					 *
	...
	function opt_in(
		...
		if ( empty( $license_key ) ) {
			...
			if ( ! $is_uninstall ) {
				$fs_user = self::_get_user_by_email( $email );
	...
	function _install_with_new_user() {
		...
		if ( ( $this->is_plugin() && fs_request_is_action( $this->get_unique_affix() . '_activate_new' ) ) ||
			...
			} elseif ( $has_pending_activation_confirmation_param ) {
	...
	private function activate_parent_account( Freemius $parent_fs ) {
		...
		$parent_install = $this->get_api_user_scope()->call(
			...
			'post',
			$parent_fs->get_install_data_for_api(
				array(
		...
		if ( ! $is_site_license_synced ) {
			...
			if ( is_numeric( $site_license_id ) ) {
				...
				if ( ! $is_license_in_result ) {
					...
					$license = $api->get(
						"/licenses/{$this->_license->id}.json?license_key=" .
	...
	function _set_data_debug_mode() {
		...
		if ( 'true' === $transient_value ) {
			$stored_key = $this->_storage->get(
				! FS_User::is_valid_id( $this->_storage->last_license_user_id ) ?
				'last_license_key' :
		...
		if ( ! $this->has_paid_plan() ) {
			...
			if ( $site->plan_id != $this->_site->plan_id ||
				...
				if ( ! is_object( $this->_license ) ) {
					...
					} elseif ( $was_license_expired_before_sync ) {
						/**
							...
							$this->_admin_notices->add(
								sprintf(
									...
									$this->contact_url(
										'bug',
										sprintf(
											$this->get_text_inline( 'I have upgraded my account but when I try to Sync the License, the plan remains %s.', 'plan-did-not-change-email-message' ),
	...
	private function generate_api_blocked_notice_message_from_result( $result ) {
		$api_domains = $this->apply_filters(
			'api_domains',
			array(
				'api.freemius.com',
				'wp.freemius.com',
	...
	private function _downgrade_site() {
		...
		$this->_admin_notices->add(
			sprintf(
				$this->get_text_inline( 'Your subscription was successfully cancelled. Your %s plan license will expire in %s.', 'plan-x-downgraded-message' ),
				$plan->title,
	...
	private function update_email( $new_email ) {
		...
		$api  = $this->get_api_user_scope();
		$user = $api->call(
			"?plugin_id={$this->_plugin->id}&fields=id,email,is_verified",
			'put',
			array(
				'email'                   => $new_email,
	...
	private function update_user_name() {
		...
		$api  = $this->get_api_user_scope();
		$user = $api->call(
			"?plugin_id={$this->_plugin->id}&fields=id,first,last",
			'put',
			array(
				'name' => $name,
			)
	...
	private function verify_email() {
		...
		$api    = $this->get_api_site_scope();
		$result = $api->call(
			"/users/{$this->_user->id}/verify.json",
			'put',
			array(
				'after_email_confirm_url' => $this->_get_admin_page_url(
					'account',
				...
				if ( $is_parent_plugin_action ) {
					if ( $is_network_action && ! empty( $blog_id ) ) {
						if ( $this->is_registered( true ) ) {
							...
							} elseif ( $this->toggle_site_tracking( false, $blog_id ) ) {
									$install = $this->get_install_by_blog_id( $blog_id );
	...
	function _add_tracking_links() {
		...
		} elseif ( $this->_is_network_active && ! $this->is_delegated_connection() ) {
	...
	function _customizer_register( $customizer ) {
		...
		if ( $this->is_pricing_page_visible() ) {
			...
			$customizer->add_control(
				new FS_Customizer_Upsell_Control(
					$customizer,
					'freemius_upsell',
					array(
						'fs'       => $this,
		...
		if ( $this->is_page_visible( 'contact' ) || $this->is_page_visible( 'support' ) ) {
			...
			$customizer->add_section(
				new FS_Customizer_Support_Section(
					$customizer,
					'freemius_support',
					array(
						'fs'       => $this,
	...
	function _gdpr_optin_ajax_action() {
		...
		$user_api = $this->get_api_user_scope_by_user( self::_get_user_by_email( $current_wp_user->user_email ) );
