...
class FS_Plugin_Updater {
	...
	private function filters() {
		...
		// WP 2.9+
		add_action(
			"after_plugin_row_{$this->_fs->get_plugin_basename()}",
			array(
				&$this,
	...
	function pre_set_site_transient_update_plugins_filter( $transient_data ) {
		...
		if ( ! isset( $this->_update_details ) ) {
			...
			$new_version = $this->_fs->get_update(
				...
				self::UPDATES_CHECK_CACHE_EXPIRATION,
	...
	private function get_latest_download_details( $addon_id = false, $newer_than = false, $fetch_readme = true ) {
		return $this->_fs->_fetch_latest_version( $addon_id, true, self::UPDATES_CHECK_CACHE_EXPIRATION, $newer_than, $fetch_readme );
	}
