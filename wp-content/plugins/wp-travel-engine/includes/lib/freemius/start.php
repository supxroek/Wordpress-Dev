...
if ( ! $is_current_sdk_from_active_theme ) {
	$theme_candidate_sdk_basename     = str_replace(
		$themes_directory . '/' . get_template() . '/',
		'',
		$fs_root_path
	);
...
if ( $is_current_sdk_from_active_theme ) {
	...
	$is_theme               = true;
} elseif ( $is_current_sdk_from_parent_theme ) {
...
} elseif ( ABSPATH !== $fs_active_plugins->abspath ) {
		/**
...
if ( ! function_exists( 'fs_find_direct_caller_plugin_file' ) ) {
	require_once __DIR__ . '/includes/supplements/fs-essential-functions-1.1.7.1.php';
}

if ( ! function_exists( 'fs_get_plugins' ) ) {
	require_once __DIR__ . '/includes/supplements/fs-essential-functions-2.2.1.php';
}
...
if ( ! isset( $fs_active_plugins->newest ) ) {
	...
	} elseif ( $is_newest_sdk_module_active &&
			$this_sdk_relative_path == $fs_active_plugins->newest->sdk_path &&
			( $fs_active_plugins->newest->in_activation ||
