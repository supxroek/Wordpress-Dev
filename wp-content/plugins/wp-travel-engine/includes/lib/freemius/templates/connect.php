...
if ( ! defined( 'ABSPATH' ) ) {
	 ...
	 */
	echo "<style>\n";
	require WP_FS__DIR_CSS . '/admin/connect.css';
	echo "</style>\n";
...
?>
	<div id="fs_connect"
			...
			if ( $is_network_upgrade_mode ) {
				...
				if ( $is_premium_code ) {
					...
					$message .= '<br><br>' . sprintf(
						fs_text_inline( 'If you\'d like to use the %s on those sites, please enter your license key below and click the activation button.', 'connect_message_network_upgrade-premium-activate-license', $slug ),
						$is_premium_only ? $fs->get_module_label( true ) : sprintf(
						/* translators: %s: module type (plugin, theme, or add-on) */
		...
		<div class="fs-terms">
			...
			if ( $require_license_key ) :
				?>
				<span class="fs-tooltip" style="width: 170px"><?php echo esc_html( $fs->get_text_inline( 'Freemius is our licensing and software updates engine', 'permissions-extensions_desc' ) ); ?></span><?php endif ?></a>
			&nbsp;&nbsp;-&nbsp;&nbsp;
