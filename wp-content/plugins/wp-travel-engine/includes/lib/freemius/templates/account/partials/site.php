...
?>
	<tr class="fs-site-details" data-blog-id="<?php echo $blog_id; ?>"
	<?php
	if ( $is_registered ) :
		?>
		data-install-id="<?php echo $install->id; ?>"<?php endif ?>>
				...
				>
					...
					<td><?php if ( ! FS_Plugin_License::is_valid_id( $install->license_id ) ) : ?>
						...
						<form action="<?php echo $fs->_get_admin_page_url( 'account' ); ?>" method="POST">
							...
							<input type="hidden" name="blog_id" value="<?php echo $blog_id; ?>">
							<button class="fs-toggle-tracking button button-small
							<?php
							if ( $is_disconnected ) {
								echo ' button-primary';
							}
