...
>
	...
	<?php if ( $is_addon_connected ) : ?>
		...
		if ( $is_addon_activated ) {
			...
			} elseif ( $fs->is_allowed_to_install() ) {
					$buttons[] = sprintf(
		...
		if ( $show_upgrade ) {
			$buttons[] = sprintf(
				...
				esc_url(
					network_admin_url(
						'plugin-install.php?fs_allow_updater_and_dialog=true' . ( ! empty( $fs_blog_id ) ? '&fs_blog_id=' . $fs_blog_id : '' ) . '&tab=plugin-information&parent_plugin_id=' . $fs->get_id() . '&plugin=' . $addon_info['slug'] .
