						...
						<li class="fs-card fs-addon" data-slug="<?php echo $addon->slug; ?>">
							<?php
								$view_details_link = sprintf(
									...
									esc_url(
										network_admin_url(
											'plugin-install.php?fs_allow_updater_and_dialog=true' . ( ! empty( $fs_blog_id ) ? '&fs_blog_id=' . $fs_blog_id : '' ) . '&tab=plugin-information&parent_plugin_id=' . $fs->get_id() . '&plugin=' . $addon->slug .
							...
							<div class="fs-inner">
								<ul>
									<li class="fs-card-banner"
										style="background-image: url('<?php echo $addon->info->card_banner_url; ?>');">
										<?php
