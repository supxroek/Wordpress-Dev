...
if ( ! defined( 'ABSPATH' ) ) {
	...
	$reconnect_url = $fs->get_activation_url(
		array(
	...
	if ( ! $permission_manager->is_premium_context() ) {
		$optional_permissions = array(
			$permission_manager->get_extensions_permission(
				false,
				false,
				true
			),
		...
		$permission_groups = array(
			array(
				...
				'prompt'              => array(
					...
					sprintf(
						$fs->esc_html_inline(
							'By clicking "Opt Out", %s will no longer be able to view your name and email.',
			...
			array(
				...
				'prompt'              => array(
					sprintf(
						$fs->esc_html_inline(
							'Sharing diagnostic data helps to provide additional functionality that\'s relevant to your website, avoid WordPress or PHP version incompatibilities that can break the website, and recognize which languages & regions the %s should be translated and tailored to.',
							'opt-out-message-clicking-opt-out'
						),
						$fs->get_module_type()
					),
					sprintf(
						$fs->esc_html_inline(
							'By clicking "Opt Out", diagnostic data will no longer be sent to %s.',
							'opt-out-message-clicking-opt-out'
						),
						$plugin_title
					),
		...
		$permission_groups = array(
			array(
				...
				'desc'                => sprintf(
					$fs->esc_html_inline(
						'For automatic delivery of security & feature updates, and license management & protection, %s needs to:',
			...
			array(
				...
				'desc'        => sprintf(
					$fs->esc_html_inline(
						'For ongoing compatibility with your website, you can optionally allow %s to:',
		...
		<div class="fs-opt-out-permissions">
			<div class="fs-modal-body">
				...
				<?php foreach ( $permission_groups as $i => $permission_group ) : ?>
					...
					<?php
					if ( $i < count( $permission_groups ) - 1 ) :
						?>
						<hr><?php endif ?>
				<?php endforeach ?>
				</div>
